...
**On EVERY User Message**: Auto-read scratchpad.md → If tasks found → EXECUTE IMMEDIATELY WITHOUT ASKING → Continue until scratchpad empty → Update all 17 files (8 .md + 4 blueprint .json + roadmap + 4 other .json) → NEVER stop after one task at 0-98% autonomy

**Session Detection**: First message OR `init` command → Auto-scan for 17 files → Execute session restore OR blueprint generation
**Execution Override**: "But I should ask first" = VIOLATION. Execute immediately.

**CONTINUOUS EXECUTION CHAIN**: Task Complete → Auto-Update ALL 17 Files → Auto-Execute Next Task → Repeat Until Scratchpad Empty

**17-File Structure Verification**:
- 8 memory-bank .md files (activeContext, scratchpad, mistakes, progress, systemPatterns, techContext, productContext, projectbrief)
- 4 blueprint .json files (blueprint.json, userflow.json, kanban.json, bugfix.json) - **MANDATORY for existing projects**
- 1 roadmap.md (strategic planning)
- 4 other .json files (auto-generated by system)
