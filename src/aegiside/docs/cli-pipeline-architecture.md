...
# CLI Pipeline (constitutional compliance)
# READ: @mcp:json-jq query '.field' from 'file.json'
...
log_and_execute "@mcp:json-jq query '.total_rl_score' from 'progress.json'"
```
...
```bash
# Constitutional mandate - ALWAYS use @mcp:json-jq for JSON reads
@mcp:json-jq query '.priority_queue[0].title' from "$memory_bank"scratchpad.json
```
...
```bash
# Constitutional mandate - use @mcp:json-jq for router queries
memory_bank=$(@mcp:json-jq query '.system_paths.memory_bank' from 'context-router.json')
rl_rewards=$(@mcp:json-jq query '.rl_calculation.rewards' from 'context-router.json')
schema_files=$(@mcp:json-jq query '.schema_files[]' from 'context-router.json')

# All queries use MCP for constitutional compliance
```
...
backup_and_update() {
  ...
  local backup=$(@mcp:json-jq query '.' from "$file")
...
# Load router using constitutional MCP
memory_bank=$(@mcp:json-jq query '.system_paths.memory_bank' from 'context-router.json')
schema_files=$(@mcp:json-jq query '.schema_files[]' from 'context-router.json')
